...
function Ep15_Progress(SelectSyntactic, objIndex)
	...
	if QS == 1 then
		if SelectSyntactic == 1 then-- Accept 
			...
			if CharClass == CLASS_KNIGHT then--Dark Knight
	...
	if QS == 5 then
		...
		if SelectSyntactic == 2 then-- I don't want to~
...
function Ep15_Complete(objIndex)
	...
	if QS == 2 then
		...
		LuaQuestClass.SendQuestComplete(Ep15, QS, objIndex)--When the confirm button is pressed without in-progress reward.
		return
	elseif QS == 10 then
	...
	local bComplete = LuaQuestClass.IsQuestComplete(Ep15, objIndex)--Completion check
