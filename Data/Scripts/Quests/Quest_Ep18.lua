...
function Ep18_Progress(SelectSyntactic, objIndex)
	...
	elseif QS == 3 then
		if SelectSyntactic == 1 then-- Accept 
			...
			if CharClass == CLASS_KNIGHT or CharClass == CLASS_MAGUMSA or CharClass == CLASS_DARKLORD or CharClass == CLASS_MONK then--Dark Knight, Magic Gladiator, Dark Lord
				LuaQuestClass.SendQuestProgress(Ep18, 4, objIndex)-- 
				return
			end
			
			if CharClass == CLASS_WIZARD then--Dark Wizard
				LuaQuestClass.SendQuestProgress(Ep18, 5, objIndex)-- 
				return
			end
			
			if CharClass == CLASS_ELF then--Elf
				LuaQuestClass.SendQuestProgress(Ep18, 6, objIndex)-- 
				return
			end
			
			if CharClass == CLASS_SUMMONER then--Summoner
	...
	elseif QS == 15 then
		if SelectSyntactic == 1 then-- Accept 
			...
			if CharClass == CLASS_KNIGHT or CharClass == CLASS_MAGUMSA or CharClass == CLASS_DARKLORD or CharClass == CLASS_ELF or CharClass == CLASS_MONK then--Dark Knight, Magic Gladiator, Dark Lord, Elf
				LuaQuestClass.SendQuestProgress(Ep18, 16, objIndex)-- 
				return
			end
			
			if CharClass == CLASS_WIZARD or CharClass == CLASS_SUMMONER then--Dark Wizard, Summoner
...
-- Enter all selection statements~
...
function Ep18_Complete(objIndex)
	...
	if QS == 2 then
		LuaQuestClass.SetQuestSwitch( Ep18, 0, objIndex)
		LuaQuestClass.SendQuestComplete( Ep18, QS, objIndex)--When the confirm button is pressed without in-progress reward.
		return
	elseif QS == 8 then
		LuaQuestClass.SetQuestSwitch( Ep18, 3, objIndex)
		LuaQuestClass.SendQuestComplete( Ep18, QS, objIndex)--When the confirm button is pressed without in-progress reward.
	...
	local bComplete = LuaQuestClass.IsQuestComplete( Ep18, objIndex)--Completion check
