...
[ Version ] 0.0.0.3
[ Text ]    Fix: commission complete being output repeatedly after daily quest completion
[ Line ]    415 - 416                               
--]]
...
function Ep20_Progress(SelectSyntactic, objIndex)
	...
	elseif QS == 2 then
		if SelectSyntactic == 1 then-- Accept 
			...
			return 
		elseif SelectSyntactic == 2 then-- Decline
	...
	elseif QS == 70 then-- Receive the 2nd-level Commission
		if SelectSyntactic == 1 then-- Accept 
...
--Create branches here
	...
	DebugPrint('QS4:', QS)
-- Level 400 or below weekday quest - 1st-level Commission
	elseif QS == 7 then
	...
	elseif QS == 19 then
		...
		LuaQuestClass.DeleteInvenItem(ItemType, ItemIndex, ItemLevel, ItemSkill, ItemCnt, Opt, ExOpt, objIndex)--Stardust

		LuaQuestClass.SetQuestSwitch(Ep20, 20, objIndex)

	elseif QS == 22 then
		ItemType = 14 ItemIndex = 154 ItemLevel = 0 ItemSkill = 0 ItemCnt = 10 Opt = 0 ExOpt = 0
		LuaQuestClass.DeleteInvenItem(ItemType, ItemIndex, ItemLevel, ItemSkill, ItemCnt, Opt, ExOpt, objIndex)--Calttstone
		LuaQuestClass.SetQuestSwitch(Ep20, 59, objIndex)
	-- Wednesday quest start
	elseif QS == 23 then
		DebugPrint('Here?')
	...
	elseif QS == 40 then
		...
		LuaQuestClass.DeleteInvenItem(ItemType, ItemIndex, ItemLevel, ItemSkill, ItemCnt, Opt, ExOpt, objIndex)--Tantalos Armor
		LuaQuestClass.SetQuestSwitch(Ep20, 41, objIndex)

	elseif QS == 43 then
		ItemType = 14 ItemIndex = 156 ItemLevel = 0 ItemSkill = 0 ItemCnt = 10 Opt = 0 ExOpt = 0
		LuaQuestClass.DeleteInvenItem(ItemType, ItemIndex, ItemLevel, ItemSkill, ItemCnt, Opt, ExOpt, objIndex)--Slaughterer's Club
		LuaQuestClass.SetQuestSwitch(Ep20, 59, objIndex)
	-- Saturday quest start
	elseif QS == 44 then
	...
	elseif QS == 60 then
		...
		LuaQuestClass.DeleteInvenItem(ItemType, ItemIndex, ItemLevel, ItemSkill, ItemCnt, Opt, ExOpt, objIndex)--1st-level Commission Completion Certificate
		LuaQuestClass.SetQuestSwitch(Ep20, 61, objIndex)
		LuaQuestClass.SetQuestTimeLimit(Ep20, 61, objIndex, 1440)
	elseif QS == 64 then
		ItemType = 14 ItemIndex = 151 ItemLevel = 0 dur = 0 ItemSkill = 0 ItemCnt = 1
		LuaQuestClass.DeleteInvenItem(ItemType, ItemIndex, ItemLevel, ItemSkill, ItemCnt, Opt, ExOpt, objIndex)--1st-level Commission
		LuaQuestClass.SetQuestSwitch(Ep20, 65, objIndex)

-- Acheron Daily Quest (2nd-level Commission)
	...
	elseif QS == 103 then
		...
		LuaQuestClass.DeleteInvenItem(ItemType, ItemIndex, ItemLevel, ItemSkill, ItemCnt, Opt, ExOpt, objIndex)-- 2nd-level Commission Completion Certificate
...
function Ep20_Complete(objIndex)
	...
	if QS == 3 then
		...
		LuaQuestClass.SendQuestComplete( Ep20, QS, objIndex)--When the confirm button is pressed without in-progress reward.
		return
	elseif QS == 6 then
		LuaQuestClass.SetQuestSwitch( Ep20, 5, objIndex)
		LuaQuestClass.SendQuestComplete( Ep20, QS, objIndex)--When the confirm button is pressed without in-progress reward.
	...
	elseif QS == 57 then
		...
		return
--  elseif QS == 61 then
--      LuaQuestClass:SendQuestConfirmBtn( Ep20, 61, objIndex)--When the confirm button is pressed without in-progress reward.
--      return
	elseif QS == 62 then
...
--  elseif QS == 104 then
--      LuaQuestClass:SendQuestConfirmBtn( Ep20, 104, objIndex)--When the confirm button is pressed without in-progress reward.
--      return
	elseif QS == 105 then
		LuaQuestClass.SetQuestSwitch( Ep20, 104, objIndex)
		LuaQuestClass.SendQuestComplete( Ep20, QS, objIndex)--When the confirm button is pressed without in-progress reward.
	...
	local bComplete = LuaQuestClass.IsQuestComplete( Ep20, objIndex)--Completion check
