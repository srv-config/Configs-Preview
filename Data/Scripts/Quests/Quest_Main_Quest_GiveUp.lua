...
[ Author ]  Kim Jeongmin
[ Version ] 0.0.0.6
[ Text ]    Fix: QS being reset to 64 when abandoning QS 72 quest
[ Line ]    EP 3        
--]]
--================================================================================================================
...
function EP6_GiveUp_Level_check(iObjIndex, Ep, QS)
	...
	if CharMaster == 0 then 
		...
		elseif CharClass == CLASS_MAGUMSA or CharClass == CLASS_DARKLORD then-- Dark Lord, Magic Gladiator
...
function EP8_GiveUp_Level_check(iObjIndex, Ep, QS)
	...
	if QS >= 0 and QS <= 5 then-- When abandoning between QS 0 ~ 5
			LuaQuestClass.SetQuestSwitch(Ep8, 0, iObjIndex)-- Set QS to the beginning of the quest branch
...
-- Hunting Ground 1 quest abandon
	elseif QS >= 27 and QS <= 29 then
		if UserLv < 400 then
...
-- Hunting Ground 2 quest abandon
...
-- Hunting Ground 1, 2 Repeatable Quest abandon
...
function EP20_GiveUp_Level_check(iObjIndex, Ep, QS)
	...
	if UserLv < 401 then
		...
		-- Cannot abandon Monday quest on the same day
		...
		elseif QS >= 16 and QS <= 19 then
			LuaQuestClass.SetQuestSwitch(Ep20, 5, iObjIndex)
		elseif QS >= 20 and QS <= 22 then
		-- Cannot abandon Tuesday quest on the same day
		...
		elseif QS >= 23 and QS <= 26 then
			LuaQuestClass.SetQuestSwitch(Ep20, 5, iObjIndex)
		elseif QS >= 27 and QS <= 29 then
		-- Cannot abandon Wednesday quest on the same day
		...
		elseif QS >= 30 and QS <= 33 then
			LuaQuestClass.SetQuestSwitch(Ep20, 5, iObjIndex)
		elseif QS >= 34 and QS <= 36 then
		-- Cannot abandon Thursday quest on the same day
			if DayOfWeek == Thu then
			DebugPrint("Thursday quest abandon: it is Thursday, cannot abandon quest")
				LuaQuestClass.SetQuestSwitch(Ep20, 34, iObjIndex)
			else
			DebugPrint("Thursday quest abandon: it is not Thursday, quest can be abandoned")
				LuaQuestClass.SetQuestSwitch(Ep20, 5, iObjIndex)
		...
		-- Cannot abandon Friday quest on the same day
		...
		-- Cannot abandon Saturday quest on the same day
		...
		-- Cannot abandon Sunday quest on the same day
	...
	elseif UserLv >= 401 then
		...
		elseif QS >= 74 and QS <= 77 then-- Monday quest
			LuaQuestClass.SetQuestSwitch(Ep20, 70, iObjIndex)
		elseif QS >= 78 and QS <= 81 then-- Tuesday quest
			LuaQuestClass.SetQuestSwitch(Ep20, 70, iObjIndex)
		elseif QS >= 82 and QS <= 85 then-- Wednesday quest
			LuaQuestClass.SetQuestSwitch(Ep20, 70, iObjIndex)
		elseif QS >= 86 and QS <= 89 then-- Thursday quest
			LuaQuestClass.SetQuestSwitch(Ep20, 70, iObjIndex)
		elseif QS >= 90 and QS <= 93 then-- Friday quest
			LuaQuestClass.SetQuestSwitch(Ep20, 70, iObjIndex)
		elseif QS >= 94 and QS <= 97 then-- Saturday quest
			LuaQuestClass.SetQuestSwitch(Ep20, 70, iObjIndex)
		elseif QS >= 98 and QS <= 101 then-- Sunday quest
			LuaQuestClass.SetQuestSwitch(Ep20, 70, iObjIndex)
		elseif QS >= 102 and QS <= 103 then-- Completed quest
